...
```bash
mv hello.txt hi.txt # hello.txt를 hi.txt로 이름을 변경
```
...
서로 다른 기기들간의 데이터 교환을 원활하게 수행할 수 있도록 표준화해놓은 통신규약이다. (컴퓨터의 공통 언어)

프로토콜은 어떤 OSI 모델 계층에 속하는 지가 중요하다.

### OSI 모델

네트워크 통신을 7개의 계층으로 나누는 프레임워크이다.

계층 별로 모든 기술은 특정 기능을 제공하고 해당 기능을 수행해야 네트워크에 유용하게 사용이 가능하다.
...
```sql
CREATE TABLE 테이블명(
    컬럼 데이터타입 NULL허용 기본값 PK칼럼
)
```

기본 값은 NULL이며 NULL은 생략해도 된다.

`ON UPDATE CURRENT_TIMESTAMP`란 데이터가 바뀔 때마다 자동으로 원하는 컬럼에 현재 시간으로 자동 업데이트 해주는 기능이다.
...
식별관계: 자식 테이블의 PK에 부모 테이블의 PK가 포함되어야 함

비식별관계: 자식 테이블의 PK는 독립적이며, 부모 테이블의 PK는 별도 FK로만 존재한다.

### Trigger (트리거)

테이블에서 어떤 이벤트가 발생했을 때 자동으로 실행되는 것을 말한다.

어떤 테이블에서 특정한 이벤트가 발생했을 때, 실행시키고자 하는 추가 쿼리 작업들이 자동으로 수행할 수 있게끔 트리거를 미리 설정해둔다.

after or before 트리거: 트리거가 실행되는 시기를 저장한다.
...
쿼리 이벤트 작동하기 전에 (미리 데이터를 확인 가능하다.)
...
특정 작업을 수행하기 위해 일련의 명령어들을 모아놓은 것을 의미한다. DB 내부에 SQL 명령문 (INSERT, SELECT, DELETE, UPDATE)의 수행로직을 하나의 함수처럼 실행한다.

하나의 요청으로 여러 SQL 문을 실행 가능하다.

그러나 문자, 숫자 연산이 들어가면 성능이 느려진다.

이러한 프로시저는 복잡한 비즈니스 로직이나 자동화된 작업 등에 사용된다.
...
자식의 FK가 부모의 PK를 참조하지만 PK에 포함이 안됨

표현법: 점선 또는 가는 실선

## 정규화 데이터베이스

중복을 최소화하도록 설계된 데이터베이스다.

같은 데이터는 DB 내에 하나만 둔다. but JOIN을 많이 하게 되는 단점이 존재한다.

## 비정규화 데이터베이스

읽는 시간을 최적화하도록 설계된 데이터베이스다.

하나 이상의 테이블에 데이터를 중복해 배치하는 최적화 기법이다. JOIN을 많이 안하게 된다.

## Redis

고성능 Key-Value 구조의 저장소

사용자가 늘어난다면 DB가 과부하가 될 수 있기 때문에 캐시 서버를 도입하여 사용한다.

캐시는 한번 읽어온 데이터를 임의의 공간에 저장하여 다음에 읽을 때는 빠륵 결괏값을 받을 수 있도록 도와주는 공간이다.
