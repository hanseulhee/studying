...
var은 선언 전과 후가 같다. var은 선언과 동시에 값을 할당한다.

let과 const의 경우 var과 다르게 변수의 선언 단계와 초기화 단계 사이에 일시적 사각 지대(TDZ)가 존재하고, TDZ에서 관리 중일 때(let 변수의 선언 또는 const 변수의 선언 및 할당 코드가 나오기 전)에 사용하려 한다면 ReferenceError 발생한다.
