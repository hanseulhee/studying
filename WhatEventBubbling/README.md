...
컨텐츠 변경이 없고 실시간 데이터가 아니며 서버측 리소스가 필요하지 않을 때 사용한다.
...
- 자바스크립트에서 지원하지 않는 다양한 기능 제공으로 생상성이 높다.
...
함수형 컴포넌트에서도 상태 관리를 할 수 있는 useState, 렌더링 직후 작업을 설정하는 useEffect 등의 기능들을 제공해 기존의 함수형 컴포넌트에서는 할 수 없었던 다양한 작업을 할 수 있게 해준다.

리액트 16.8v에 나온 기능

## useEffect 의존성 배열에서 변경이 일어났는지 리액트는 어떻게 판단해요?

리액트는 의존성 배열에 포함된 값들을 얕은 비교를 하여 변경 여부를 확인합니다.

배열 안에 객체나 배열과 같은 참조 타입이 포함되어 있다면, 해당 객체나 배열의 참조가 변경되었는지를 비교합니다. 객체나 배열의 내부 값이 변경되었다고 해도 참조 자체는 변경되지 않았다면 리액트는 해당 값이 변경되지 않은 것으로 간주합니다.

## nextjs에서 서버사이드 렌더링을 하기 위해선 어떤 메소드가 필요한가?

getServerSideProps가 필요하다.

## getServerSideProps는 반환값이 무엇인가?
...
타입 단언은 강제적으로 as 키워드를 사용해 타입을 지정하는 것을 의미하고 타입 선언은 명시적으로 지정하는 것으로 타입 추론으로도 가능은 하지만 명시적으로 함으로써 코드의 안정성과 가독성을 높입니다.
...
바벨은 최신 ES6+ 문법을 ES5로 바꾸어주는 것이고, 폴리필은 브라우저가 이해할 수 없는 코드에 대하여 이해할 수 있는 코드 소스로 제공한다.

ES5 환경의 전역 객체에 존재하지 않는 객체, 메서드, 함수는 바벨이 컴파일해줄 수 없고 폴리필이 필요하다.
...
몹 프로그래밍은 전체 팀이 동일한 공간에서 동일한 컴퓨터로 동일한 작업을 수행하는 개발 방식이다.
