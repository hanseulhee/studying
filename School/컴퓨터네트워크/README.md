...
네트워크에서 전달되는 최대 전송 길이를 MTU(Maximum Transfer Unit)라고 한다.
MTU 길이에 따라 나누어 보내는 것을 **‘단편화’** 라고 한다.

단편화와 재조립과 관련된 필드는 **식별자, 플래그, 단편화 옵셋**이다.

데이터그램이 프레임 속에 캡슐화 될 때 데이터그램의 크기는 이 MTU 크기보다 작아야 한다.

### 식별자

16비트로 구성된다.

발신지 호스트로부터 나온 데이터그램을 유일하게 식별한다.

식별자와 발신지 IP 주소의 조합은 데이터그램이 발신지 호스트를 떠날 때 유일하게 정의할 수 있다.

### 플래그

3비트로 구성되는데 처음 비트는 사용되지 않는다.
두 번째 비트는 D(Do not fragment) : 값이 1이면 단편화하지 말아라.
세 번째 비트는 M(More fragments) : 값이 1이면 뒤에 따라오는 데이터그램이 더 있다. 즉 0이어야 뒤의 데이터그램이 없고, 값이 1이면 데이터그램은 마지막 단편이 아니다.

### 단편화 옵셋

13비트로 구성된다.

전체 데이터그램 내에서 단편의 상대적 위치를 나타낸다.

원래의 데이터그램 내에서 데이터의 옵셋을 **8바이트 단위** 로 나타낸다.

## 옵션

헤더의 고정 부분과 가변 부분 중 가변 부분은 옵션으로 구성되어있다.

최대 길이는 40바이트이다.

옵션은 네트워크를 시험하거나 디버그하기 위해 사용한다.

유형(type) 필드와 길이 필드, 그리고 가변 길이의 값 필드로 구성되어 있으며 종종 TLV(Type-Length-Value)라고도 한다.

옵션은 단일 바이트와 다중 바이트로 나뉜다

단일 바이트는 length도 없고 값도 없다.

무연산 옵션은 1바이트 옵션으로 옵션들 사이의 여백을 채워준다.

옵션 종료는 1바이트고 옵션의 필드 끝에 패딩의 목적으로 사용한다. 오직 하나의 옵션 종료만 사용이 가능하다.
