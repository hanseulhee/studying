...
- 송신자는 호스트이고 같은 네트워크상에 다른 호스트에 패킷 전송(논리
  주소는 목적지 IP주소)

- 송신자는 호스트이고 다른 네트워크상에 있는 어떤 호스트에게 패킷 전
  송(논리 주소는 라우터의 IP 주소)

- 다른 네트워크상에 있는 호스트로 가는 데이터그램을 수신한 라우터가
  송신자(논리 주소는 다음 홉 라우터의 IP 주소)

- 같은 네트워크 상에 있는 호스트로 가는 데이터그램을 수신한 라우터가
  송신자(논리 주소는 목적지 IP 주소)
